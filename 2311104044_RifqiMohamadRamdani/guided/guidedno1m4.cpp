...
void insertDepan(const mahasiswa &data) {
    Node *baru = new Node;
    strcpy(baru->data.nama, data.nama);  // Menggunakan strcpy untuk string
    strcpy(baru->data.nim, data.nim);    // Menggunakan strcpy untuk string
    baru->next = nullptr;
    if (isEmpty()) {
        head = tail = baru;
    } else {
...
int main() {
    ...
    return 0;  // Menggunakan return 0 dengan benar
}
